...
namespace SoftFluent.Windows {
   public static class ConversionHelper {
      public static List<T> SplitToList<T>(this string thisString, params char[] separators) {
         ...
         if (thisString != null) {
            foreach (string s in thisString.Split(separators)) {
               T item = ConversionHelper.ChangeType<T>(s);
               list.Add(item);
            }
         }
         return list;
      ...
      public static bool TryChangeType<T>(object input, IFormatProvider provider, out T value) {
         bool b = BaseConverter.TryChangeType(input, typeof(T), provider, out object v);
         if (!b) {
            if (v == null) {
      ...
      public static bool TryChangeType(object input, Type conversionType, IFormatProvider provider, out object value) {
         return BaseConverter.TryChangeType(input, conversionType, provider, out value);
      }
